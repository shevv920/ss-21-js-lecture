...
```
### Promise
- Промис это представитель для значения, которое может еще не быть вычислено на момент создания промиса. Промис позволяет назначить обработчики (колбэки) для результата или ошибки асинхронного действия.
